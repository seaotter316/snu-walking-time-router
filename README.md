...
기본 `base_walk_speed_kmh`는 6.0km/h입니다. UI에서 사용자가 실제 이동 시간을 입력해 속도 보정을 실행하면, 해당 경로의 기본 보행속도를 역산하고 이후 야외 보행 엣지 시간만 비례 조정합니다. 건물 내부 계단과 셔틀 시간은 개인 보행속도 보정 대상에서 제외됩니다.
